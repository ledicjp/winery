...
public class SchemaVisitor extends AbstractVisitor<Result, Parameter> {
	...
	public Map<QName, Map<String, QName>> visit(TServiceTemplate node, String PATH, String namespace) throws IException {
		for (TMapImportDefinition map : node.getImports()) {
			for (Map.Entry<String, TImportDefinition> entry : map.entrySet()) {
				Reader reader = new Reader();
				TServiceTemplate serviceTemplate = reader.parse(entry.getValue(), PATH, entry.getValue().getNamespace_uri());
				visit(serviceTemplate, new Parameter(PATH, entry.getValue().getNamespace_uri()).setBuildSchema(false));
			}
		}
	...
	public Result visit(TServiceTemplate node, Parameter parameter) throws IException {
		...
		this.data_types = node.getData_types();
		for (TMapImportDefinition map : node.getImports()) {
			for (Map.Entry<String, TImportDefinition> entry : map.entrySet()) {
				addImport(entry.getValue().getNamespace_uri(), new AbstractMap.SimpleEntry<>(entry));
			}
		}

		// init localDataTypesList
		node.getData_types().forEach((key, value) -> this.localDatatypeNames.add(key));
		...
		// Optimized: parameter.datatype is set and defined in this service template 
			...
			node.getData_types().get(parameter.getDatatype().getLocalPart())
		...
		for (Map.Entry<String, TPropertyDefinition> entry : node.getProperties().entrySet()) {
			...
			// if parameter.datatype is defined and property type is not in this service template
				...
				for (Map.Entry<String, TImportDefinition> importDefinition : imports.get(type.getNamespaceURI())) {
					Reader reader = new Reader();
					TServiceTemplate serviceTemplate = reader.parse(importDefinition.getValue(), parameter.getPath(), importDefinition.getValue().getNamespace_uri());
					...
					builder.addImports(importDefinition.getValue().getNamespace_uri(), getRelativeFileName(importDefinition.getValue().getNamespace_uri()));
					builder.addElements(entry.getKey(), entry.getValue());
				}
			}
	 ...
	 */
	private void visitChildes(TServiceTemplate node, Parameter parameter) throws IException {
		...
		if (node.getTopology_template() != null) {
			node.getTopology_template().accept(this, parameter.copy().addContext("topology_template"));
		}
	}
}
