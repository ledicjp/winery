...
public class Printer extends AbstractResult<Printer> {
            ...
            .print(value.stream()
                .map((object) -> {
                    if (object instanceof QName) {
                        return QName2String((QName) object);
                    } else {
    ...
    public Printer printKeyValue(String key, QName value) {
        ...
        return printKeyValue(key, QName2String(value));
    }
    ...
    private String QName2String(QName name) {
        if (!name.getPrefix().isEmpty() && (
            name.getPrefix().equals("yaml")
                || name.getPrefix().equals("tosca")
        )) return name.getLocalPart();
        return (name.getPrefix().isEmpty() ? "{" + name.getNamespaceURI() + "}" : name.getPrefix() + ":") + name.getLocalPart();
    }
